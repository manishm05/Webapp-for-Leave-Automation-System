<?php
	...
	if($_GET['reject'] != NULL) { 
	$sql1="delete from apply_leave where leave_id=".$_GET['reject'];
	$result1=mysqli_query($conn, $sql1);
	echo  "leave_id".$_GET['reject']."has been rejected ";
	header("location: employee_pending_request.php");
	}
	?>
<!DOCTYPE html>
<html lang="en">
  <head>
    ...
    <title>Employee Area | View Pending Request</title>
  ...
  <body>
      ...
      <div class="container">
        ...
        <div id="navbar" class="collapse navbar-collapse">
          <ul class="nav navbar-nav">
			...
			<li class="active"><a href="http://localhost/registration/employee_pending_request.php">Pending</a></li>
    ...
    <header id="header">
      <div class="container">
          <div class="row">
            <div class="col-md-10">
                <h1><span class="glyphicon glyphicon-list" aria-hidden="true"></span> Dashboard<small>Pending</small></h1>
            </div> 
          </div>
      </div>
    </header>
...
//{
 ...
 $myid_1= $_COOKIE["id"];
	...
	$sql="select leave_id, type_of_leave, concerned_manager,time_of_leave, date, date_till, count_leave, reason from apply_leave where user_id='$myid_1' and status='pending'";
			...
			while($user=mysqli_fetch_assoc($record))
	{
		...
		echo "<td>"."<a href=employee_pending_request.php?reject=".$user['leave_id']." class=btn name=reject >CANCEL</a> "."</td>";
		echo "</tr>";
	//}
} 
	?>
